...
def _show_start_form() -> None:
    st.subheader("Configure AutoTrader")
    ...
    st.caption(f"{len(tickers)} tickers selected: {', '.join(tickers)}")

    if st.button("Start AutoTrader", type="primary", disabled=not tickers):
        _start_trader(tickers, interval, dry_run, market_hours, state_path)
        st.rerun()


def _show_running_dashboard() -> None:
    # Drain the queue into the reports list
    ...
    with col_status:
        st.success(f"AutoTrader RUNNING — {len(reports)} cycles completed")
    with col_stop:
        if st.button("Stop AutoTrader", type="secondary"):
    ...
    if trader:
        ...
        if portfolio:
            ...
            with col3:
                metric_card("Return", f"{ret_pct:+.2f}%")
    ...
    if reports:
        ...
        st.subheader(f"Last Cycle ({finished.strftime('%H:%M:%S') if finished else '—'})")
        if actions:
            rows = [{
                "Ticker": getattr(a, "ticker", ""),
                "Action": getattr(a, "action", ""),
                "Direction": getattr(a, "direction", ""),
                "Confidence": f"{getattr(a, 'confidence', 0):.1%}",
